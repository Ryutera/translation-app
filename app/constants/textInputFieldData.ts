export const TRANSLATION_UI = {
  Japanese: {
    placeholder: "翻訳したい文章を入力...",
    translate: "翻訳 (ほんやく)",
    remainingFree: "無料翻訳",
    remainingDaily: "本日の残り",
    limitReached: "利用制限に達しました",
    limitDaily: "本日の利用制限に達しました",
    resetNotice: "(24時間後に再び利用できます)",
    upgradePremium: "プレミアム登録をして利用回数を増やす",
    loginToIncrease: "ログインして利用回数を増やす",
  },
  English: {
    placeholder: "Type something to translate...",
    translate: "Translate",
    remainingFree: "Free Trial",
    remainingDaily: "Daily Quota",
    limitReached: "Limit reached",
    limitDaily: "Daily limit reached",
    resetNotice: "(Resets in 24 hours)",
    upgradePremium: "Upgrade to Premium for unlimited access",
    loginToIncrease: "Sign in to get more translations",
  },
  Korean: {
    placeholder: "번역할 내용을 입력하세요...",
    translate: "번역하기",
    remainingFree: "무료 체험",
    remainingDaily: "오늘의 잔여 횟수",
    limitReached: "이용 한도에 도달했습니다",
    limitDaily: "오늘의 이용 한도를 모두 사용했습니다",
    resetNotice: "(24시간 후에 다시 이용 가능합니다)",
    upgradePremium: "프리미엄 등록하고 횟수 늘리기",
    loginToIncrease: "로그인하고 더 많은 횟수 받기",
  },
  Chinese: {
    placeholder: "请输入要翻译的句子...",
    translate: "立即翻译",
    remainingFree: "免费试用",
    remainingDaily: "今日剩余次数",
    limitReached: "已达到使用限制",
    limitDaily: "已达到今日使用上限",
    resetNotice: "(24小时后将重新开放)",
    upgradePremium: "加入高级版以增加次数",
    loginToIncrease: "登录以获得更多使用次数",
  }
};